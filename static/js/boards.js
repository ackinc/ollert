...
fetch(BOARDS_API_URL, {
  ...
  .then((body) => {
    if (body.error === "NOT_AUTHENTICATED") {
      ...
      document.querySelector("main .loading").innerHTML =
...
function renderBoards(boards) {
  return boards
    .map(
      (board) => `<div class="board board-like">
...
function handleClick(e) {
  ...
  if ((node = getNearestParentWithClass(e.target, "create-board"))) {
    ...
    (node = getNearestParentWithClass(e.target, "list-item-mark-done"))
  ) {
...
function allowEditBoardName(board_node) {
  ...
  board_title_input_node.addEventListener("keyup", function (e) {
    if (e.key === "Enter" && this.value !== "")
      editBoardName(board_node, this.value);
  });
...
function editBoardName(board_node, new_name) {
  ...
  board_node.querySelector(
    ".board-content"
  ).innerHTML = `<h1> ${new_name}</h1>`;
...
function showLists(board) {
  ...
  main_node.innerHTML =
    renderLists(lists) +
    `<div id="new-list-control-div">${create_list_html}</div>`;
...
function renderLists(lists) {
  return lists
    .map(
      (list, idx) => `<div class="list list-like" data-idx="${idx}">
...
function addItemToList(list_node, item) {
  cur_board.lists[+list_node.dataset.idx].items.push({
    desc: item,
    done: false,
  });
...
function toggleListItemDone(list_item_node) {
  ...
  const list_idx = getNearestParentWithClass(list_item_node, "list").dataset
    .idx;

  cur_board.lists[list_idx].items[item_idx].done = !cur_board.lists[list_idx]
    .items[item_idx].done;
...
function getBoardIndex(board_node) {
  let idx = 0,
    cur = main_node.querySelector(".board");
...
function getNearestParentWithClass(node, req_class) {
  ...
  while (
    node !== null &&
    !Array.prototype.includes.call(node.classList || [], req_class)
  ) {
